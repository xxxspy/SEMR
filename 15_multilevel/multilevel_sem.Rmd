...
上图是随机截距的模型图, 注意区分矩形和圆形, 因为矩形代表的是显变量, 圆形代表的是潜变量.

该示例与示例之前的模型的区别在于该模型同时具有随机截距和随机斜率。 
在模型的"Within"部分，从 x 到 y 的箭头末端的实心圆表示随机截距，在模型的"Between"部分的潜变量y代表随机截距。 
在模型的"Within"部分, 从 x 到 y 的箭头中间的实心圆表示随机斜率, 因为斜率是随机的, 所以它是一个变量, 我们赋予它一个变量名s。 
在模型的"Between"部分，随机截距和随机斜率以圆圈显示，因为它们是跨组变化的连续潜在变量。 变量 xm 是取每个组的 x 平均值。
...
在 DEFINE 命令中，个体级协变量 x 使用 x 的组均值居中。 当估计随机斜率时建议这样做（Raudenbush & Bryk, 2002, p. 143）。

在模型的内部部分，$|$ 符号与 TYPE=RANDOM 结合使用来命名和定义模型中的随机斜率。
$|$ 左侧用于设置随机斜率的名称,你可以设置任何自己觉得合适的名称。$|$ 右侧之名随机斜率发生在哪一条路径。 使用 ON 选项定义随机斜率。


在模型的"between"部分，ON 语句描述了组间变量 w 和 xm 上的随机截距 y 和随机斜率 s 的线性回归。 
估计 s 和 y 的截距和残差方差，默认情况下残差不相关。 WITH 语句指定 s 和 y 的残差相关。 
此类分析的默认估计量是具有稳健标准误差的最大似然估计。 ANALYSIS 命令的ESTIMATOR 选项可用于选择不同的估计器。 其他命令的解释可以在例 9.1 中找到。
